...
//      DONE (7) Generate a weather summary with the date, description, high and low
        ...
        // weatgerForThisDay = mWeatherData[position];
        int dateColIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_DATE);
        String weatherDate = SunshineDateUtils.getFriendlyDateString(mContext,
                    mCursor.getLong(dateColIndex), false);
        int weatherColIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID);
        String weatherDesc = SunshineWeatherUtils
                .getStringForWeatherCondition(mContext, mCursor.getInt(weatherColIndex));
        ...
        weatherForThisDay = weatherDate + " - " + weatherDesc + " - " + highLowTemp;
